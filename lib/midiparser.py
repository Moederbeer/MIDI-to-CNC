...
class File:
	def __init__(self, file):
		...
		for i in range(0, len(binSt)):
			string = string + hex(binSt[i])
...
class Event:
	...
	def read(self, prev_time, prev_status, str):
		...
		if not (str[0] & 0x80):
			...
			str = bytes(prev_status) + str

		self.status = str[0]
		self.channel = (self.status) & 0xF
	...
	# handle voice events
		...
		if meta_msg == meta.FileMetaEvent:
			...
			if type == meta.SetTempo or \
				...
				self.detail.fifths = (str[0])

				if (str[1]): self.detail.mode = "minor"
				...
				self.detail.numerator = (str[0])
				self.detail.log_denominator = (str[1])
				self.detail.midi_clocks = (str[2])
				self.detail.thirty_seconds = (str[3])
				...
				self.detail.hour = (str[0])
				self.detail.minute = (str[1])
				self.detail.second = (str[2])
				self.detail.frame = (str[3])
				self.detail.sub_frame = (str[4])
				str = str[length:]
